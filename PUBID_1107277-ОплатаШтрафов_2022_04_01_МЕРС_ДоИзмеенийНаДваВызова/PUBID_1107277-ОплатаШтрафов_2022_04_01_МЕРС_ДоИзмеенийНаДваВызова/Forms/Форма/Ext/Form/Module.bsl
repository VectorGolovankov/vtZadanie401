...
Процедура СтрокаШтрафаПриИзмененииНаСервере()
	...
	Если ЛьготнаяДата<НачалоДня(ТекущаяДата()) Тогда
		СуммаДокументаСтр=СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(МассивСтрок[18],"=",ложь)[1];
	Иначе
		СуммаДокументаСтр=СтроковыеФункцииКлиентСервер.РазложитьСтрокуВМассивПодстрок(МассивСтрок[19],"=",ложь)[1];
	КонецЕсли;
